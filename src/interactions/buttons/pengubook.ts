...
import { fetchMultipleUsernames, fetchMultipleUserData, getDiscordClient } from "../../services/discord_users.js";

// Helper function to get unread message count
export async function getUnreadMessageCount(discordId: string): Promise<number> {
  try {
...
export async function handlePenguBookNav(i: ButtonInteraction, mode: string, page: number) {
  ...
  try {
    ...
    profiles = await Promise.race([profileQuery, dbTimeout]) as any[];
...
export async function handlePenguBookModes(i: ButtonInteraction) {
  const embed = new EmbedBuilder()
    .setColor(0x5865f2)
    .setTitle("📊 PenguBook Browse Modes")
    .setDescription("Choose how you want to explore profiles:")
    .addFields(
      { name: "🕒 Recent", value: "Browse profiles by most recently updated", inline: false },
      { name: "🔥 Popular", value: "Browse profiles by most profile views", inline: false },
      { name: "🎲 Random", value: "Discover profiles randomly", inline: false }
    );
...
export async function handleBioToggle(i: ButtonInteraction, setting: string, value: boolean) {
  await i.deferUpdate();
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: "📖 Show in PenguBook", value: updatedUser!.showInPenguBook ? "✅ Enabled" : "❌ Disabled", inline: true },
        { name: "💰 Allow Tips from Book", value: updatedUser!.allowTipsFromBook ? "✅ Enabled" : "❌ Disabled", inline: true }
      );
...
export async function handleTipFromBook(i: ButtonInteraction, targetDiscordId: string) {
...
export async function handleViewOwnBio(i: ButtonInteraction) {
    ...
    const buttons = new ActionRowBuilder<ButtonBuilder>()
      .addComponents(
        ...
        new ButtonBuilder()
          ...
          .setLabel(inboxLabel)
          .setStyle(unreadCount > 0 ? ButtonStyle.Success : ButtonStyle.Secondary),
        new ButtonBuilder()
...
export async function handlePenguBookCTA(i: ButtonInteraction) {
  const modal = new ModalBuilder()
  ...
  modal.addComponents(
    ...
    new ActionRowBuilder<TextInputBuilder>().addComponents(xInput)
  );
...
export async function handlePenguBookBioSetup(i: ModalSubmitInteraction) {
  await i.deferReply({ ephemeral: true });
...
export async function handleTipModal(i: ModalSubmitInteraction, parts: string[]) {
...
export async function handlePenguBookInbox(i: ButtonInteraction) {
  await i.deferReply({ ephemeral: true });
