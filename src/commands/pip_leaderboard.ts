...
export default async function pipLeaderboard(i: ChatInputCommandInteraction) {
  try {
...
async function buildStreakLeaderboard(limit: number): Promise<EmbedBuilder> {
...
async function buildWinsLeaderboard(limit: number): Promise<EmbedBuilder> {
  ...
  if (topWinners.length === 0) {
    ...
    const entries = topWinners.map((stats: any, index: number) => {
      const rank = index + 1;
...
async function buildWinRateLeaderboard(limit: number): Promise<EmbedBuilder> {
...
async function buildTipsSentLeaderboard(limit: number): Promise<EmbedBuilder> {
  // Use caching for leaderboard data
  ...
  if (topTippers.length === 0) {
    ...
    const entries = topTippers.map((stats: any, index: number) => {
      const rank = index + 1;
      const medal = getMedal(rank);
    ...
    embed.addFields({
      name: "Most Generous",
      value: entries.filter((e: string) => e).join("\n").substring(0, 1024),
      inline: false
    });
  }
...
async function buildTipsReceivedLeaderboard(limit: number): Promise<EmbedBuilder> {
  ...
  if (topReceivers.length === 0) {
    ...
    const entries = topReceivers.map((stats: any, index: number) => {
      const rank = index + 1;
      const medal = getMedal(rank);
...
async function buildReferralLeaderboard(limit: number): Promise<EmbedBuilder> {
...
async function buildWealthLeaderboard(limit: number): Promise<EmbedBuilder> {
