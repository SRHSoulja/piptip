...
passwordStrengthRouter.post('/check-strength', (req: Request, res: Response) => {
  try {
...
passwordStrengthRouter.get('/password/:discordId', (req: Request, res: Response) => {
  try {
...
function assessPasswordStrength(password: string) {
  ...
  let level: 'very_weak' | 'weak' | 'fair' | 'good' | 'strong' | 'very_strong';
  let color: string;
  let message: string;
...
function estimateCrackTime(password: string, score: number): string {
  ...
  if (adjustedTime < 31536000) return `${Math.round(adjustedTime / 86400)} days`;
  if (adjustedTime < 31536000000) return `${Math.round(adjustedTime / 31536000)} years`;
  return 'Centuries';
}

// Render password change page with strength meter
function renderPasswordChangePage(data: { discordId: string }) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
