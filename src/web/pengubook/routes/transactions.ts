// src/web/pengubook/routes/transactions.ts - User transaction history page
...
export async function transactionsHandler(req: Request, res: Response) {
  try {
    ...
    // Add group tips
    groupTipsCreated.forEach((groupTip: any) => {
      allTransactions.push({
        type: 'group_tip',
    ...
    const totalTransactions = totalCounts.reduce((sum: any, count: any) => sum + count, 0);
            ...
            ${paginatedTransactions.length > 0 ? `
                <div class="pg-transaction-list">
                    ${paginatedTransactions.map(tx => {
                      ...
                      const config = typeConfig[tx.type as keyof typeof typeConfig] || typeConfig.tip_sent;
