...
// 2FA setup page
twoFactorRouter.get('/2fa-setup/:discordId', async (req: Request, res: Response) => {
  try {
    ...
    (req as any).session.tempSecret = secret;
...
// Verify 2FA setup
twoFactorRouter.post('/2fa-verify/:discordId', async (req: Request, res: Response) => {
  try {
    const { discordId } = req.params;
...
async function saveUserTwoFactorSecret(discordId: string, secret: string) {
...
function render2FASetupPage(data: any) {
    ...
    <div class="setup-content">
      ...
      <div class="backup-codes">
        ...
        <div class="codes-grid">
          ${backupCodes.map((code: string) => `<div class="backup-code">${code}</div>`).join('')}
        </div>
