 ...
 */
marketsApiRouter.get("/markets", async (req: Request, res: Response) => {
  try {
 ...
 */
marketsApiRouter.get("/market/:id", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
 ...
 */
marketsApiRouter.post("/bet", async (req: Request, res: Response) => {
  try {
    // Check if user is authenticated via Discord OAuth
    const discordId = (req as any).user?.discordId || (req as any).session?.discordId;
 ...
 */
marketsApiRouter.get("/user/participations", async (req: Request, res: Response) => {
  try {
    const discordId = (req as any).user?.discordId || (req as any).session?.discordId;
 ...
 */
marketsApiRouter.get("/user/balance", async (req: Request, res: Response) => {
  try {
    const discordId = (req as any).user?.discordId || (req as any).session?.discordId;
 ...
 */
marketsApiRouter.get("/stats", async (req: Request, res: Response) => {
  try {
    const [
      totalMarkets,
      activeMarkets,
