...
async function previewDisable(achievementIds: number[]) {
...
async function previewThresholdChange(achievementIds: number[], newThreshold: number) {
...
async function previewToggle(achievementIds: number[]) {
...
async function previewCategoryDisable(category: string) {
  ...
  const totalUnlocks = categoryUnlocks.reduce(
    (sum, def) => sum + def._count.unlockedAchievements, 0
  );
...
function calculateProcessingTime(preview: any): string {
  ...
  try {
    ...
    switch (operation) {
      ...
      case 'toggle_enabled':
        ...
        const togglePromises = achievementIds.map(async (id: number) => {
...
// Store rollback data
async function storeRollbackData(operation: string, achievementIds: number[], changes: any): Promise<string> {
