...
systemRouter.get("/emergency/status", async (req: Request, res: Response) => {
  try {
    const config = await prisma.appConfig.findFirst();
...
systemRouter.post("/system/grand-reset", async (req: Request, res: Response) => {
  try {
...
systemRouter.get("/system/stats", async (req: Request, res: Response) => {
  try {
...
systemRouter.get("/sync/status", async (req: Request, res: Response) => {
  try {
...
systemRouter.post("/sync/fix", async (req: Request, res: Response) => {
  try {
    const syncMonitor = getSyncMonitor(prisma);
...
systemRouter.post("/sync/validate", async (req: Request, res: Response) => {
  try {
...
systemRouter.post("/migrations/apply", async (req: Request, res: Response) => {
  try {
...
systemRouter.get("/discord/status", async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      discord: {
        ...
        rateLimiter: {
          ...
          totalQueued: Object.values(rateLimiterStatus).reduce((sum, endpoint: any) => sum + endpoint.queueLength, 0),
          processing: Object.values(rateLimiterStatus).filter((endpoint: any) => endpoint.processing).length
        },
        groupTipTimers: {
...
systemRouter.post("/discord/retry-failed", async (req: Request, res: Response) => {
  try {
...
systemRouter.post("/discord/clear-queue", async (req: Request, res: Response) => {
  try {
    const { confirmToken } = req.body;
