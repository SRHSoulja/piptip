// Admin Panel 2FA Setup - Secure admin.piptip.com access only
import type { Request, Response } from 'express';
import crypto from 'crypto';

interface Admin2FASetup {
  adminId: string;
  secret: string;
  qrCodeUrl: string;
  backupCodes: string[];
...
const admin2FAStorage = new Map<string, Admin2FASetup>();

export async function getAdmin2FASetup(req: Request, res: Response) {
  try {
...
export async function initializeAdmin2FA(req: Request, res: Response) {
  try {
...
function getAdminId(req: Request): string | null {
  ...
  return req.headers['x-admin-id'] as string || 'admin_demo_001';
}

function generateAdmin2FASetupPage(adminId: string): string {
...
function getAdmin2FAStyles(): string {
  return `
...
function getAdmin2FAScript(): string {
  return `
    let backupCodes = [];
