 ...
 */
export function updateQueueMetrics(queueName: string, stats: Record<string, number>): void {
 ...
 */
export function recordJobProcessing(queueName: string, jobType: string, status: 'success' | 'failed', duration: number): void {
 ...
 */
export function recordDiscordInteraction(command: string, status: 'success' | 'error', duration: number, guildId?: string): void {
 ...
 */
export function recordDatabaseOperation(operation: string, table: string, duration: number): void {
 ...
 */
export function recordMarketActivity(type: 'created' | 'resolved', data: {
  creatorType?: string;
  marketType?: string;
 ...
 */
export function recordBet(outcome: string, amount: number, tokenSymbol: string, guildId?: string): void {
 ...
 */
export function recordPayout(status: 'success' | 'failed', marketId: string, amount: number, tokenSymbol: string): void {
 ...
 */
export function recordBalanceDrift(severity: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL', type: string): void {
...
export function startMetricsCollection(): void {
  ...
  setInterval(async () => {
    try {
      ...
      Object.entries(health.queues).forEach(([queueName, stats]: [string, any]) => {
        if (stats.healthy) {
