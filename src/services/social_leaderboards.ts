...
export async function getSocialLeaderboard(limit: number = 10): Promise<Array<{
  ...
  try {
    ...
    const userScores = await Promise.all(
      users.map(async (user) => {
        ...
        try {
          ...
          ]) as Awaited<ReturnType<typeof calculateSocialScore>>;
          ...
          ]) as Awaited<ReturnType<typeof getUserLevel>>;
    ...
    const validScores = userScores.filter((score): score is NonNullable<typeof score> => score !== null);
