...
import { PENGUIN_SUCCESS, createPenguinSuccess } from "../utils/penguin_messages.js";
...
export async function awardTipXP(
  fromDiscordId: string,
  toDiscordId?: string,
  isGroupTip: boolean = false
): Promise<{
  senderXP: number;
  receiverXP: number;
  senderLevelUp?: boolean;
  receiverLevelUp?: boolean;
  messages: string[];
}> {
...
export async function awardGroupTipClaimXP(
  discordId: string
): Promise<{
  xpAwarded: number;
  levelUp?: boolean;
...
// Award XP for group tip claims
export async function awardXPForGroupTipClaim(discordId: string): Promise<{
  xpAwarded: number;
  levelUp?: boolean;
  newLevel?: any;
  message?: string;
...
export async function awardDepositXP(
  discordId: string,
  amount?: string
): Promise<{
  xpAwarded: number;
