...
export interface TokenPayment {
  tokenId: number;
  amount: bigint; // Atomic amount
}

export interface TournamentEntryResult {
  success: boolean;
  tpipAllocated?: bigint;
  totalUsdPaid?: number;
  payments?: Array<{
    tokenId: number;
    tokenSymbol: string;
...
export interface TournamentConfig {
  id: string;
  name: string;
  entryFeeUSD: number; // USD entry fee
  tpipAllocation: number; // TPIP given to each participant
  guildId?: string;
}
...
export async function calculateEntryPayment(params: {
  tournamentId: string;
  desiredPayments: Array<{ tokenId: number; percentage: number }>; // e.g., [{tokenId: 1, percentage: 50}, {tokenId: 3, percentage: 50}]
}): Promise<{
  success: boolean;
  payments?: Array<{
    tokenId: number;
