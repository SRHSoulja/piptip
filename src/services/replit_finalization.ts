 ...
 */
export async function checkAndFinalizeExpiredTips(client: Client): Promise<void> {
  try {
 ...
 */
export async function replitUpdateMessage(client: Client, tipId: number): Promise<boolean> {
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      await Promise.race([
        updateGroupTipMessage(client, tipId),
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error("Replit timeout")), 8000)
        )
      ]);
      return true;
    } catch (error: any) {
