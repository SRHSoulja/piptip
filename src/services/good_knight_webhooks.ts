...
interface GoodKnightWebhookConfig {
  webhookId: string;
  token: string;
  guildId: string;
  channelId: string;
...
class GoodKnightWebhookManager {
  private authorizedWebhooks = new Map<string, GoodKnightWebhookConfig>();
  private fallbackLog: Array<{ timestamp: Date; type: string; message: string; payload?: any }> = [];
   ...
   */
  private initializeAuthorizedWebhooks(): void {
   ...
   */
  private parseWebhookConfigs(): GoodKnightWebhookConfig[] {
    const configs: GoodKnightWebhookConfig[] = [];
   ...
   */
  private isWebhookAuthorized(webhookId: string, guildId: string): boolean {
   ...
   */
  async sendAuthorizedWebhook(
    type: 'alert' | 'monitoring' | 'system' | 'achievement',
   ...
   */
  private findAuthorizedWebhook(type: string, targetGuild?: string): GoodKnightWebhookConfig | null {
   ...
   */
  private addGoodKnightMetadata(payload: WebhookPayload, metadata: any): WebhookPayload {
   ...
   */
  private generateGoodKnightSignature(webhook: GoodKnightWebhookConfig, payload: WebhookPayload): string {
   ...
   */
  private logFallbackMessage(type: string, reason: string, payload: any): void {
    const logEntry = {
      timestamp: new Date(),
   ...
   */
  getFallbackLogs(): Array<{ timestamp: Date; type: string; message: string; payload?: any }> {
  ...
  getWebhookStatus(): {
    authorized: number;
    total: number;
    webhooks: Array<{ id: string; guild: string; permissions: string[]; authorized: boolean }>;
  } {
...
export type { WebhookPayload, GoodKnightWebhookConfig };

// Helper function for backward compatibility
export async function sendGoodKnightAlert(
  type: 'alert' | 'monitoring' | 'system',
  title: string,
  description: string,
