...
export class SportsResolverService {
     ...
     */
    async fetchUpcomingGames(league: string) {
        try {
            const leagues = this.getSupportedLeagues();
            const leagueId = leagues[league as keyof typeof leagues];
     ...
     */
    async fetchGameResult(eventId: string) {
        try {
     ...
     */
    async findTeamNextGame(teamName: string, league?: string) {
        try {
     ...
     */
    async resolveSportsMarket(market: any): Promise<{ outcome: 'YES' | 'NO' | 'CANCEL'; data: any }> {
        ...
        return { outcome: outcome as 'YES' | 'NO', data: resolutionData };
    }

    /**
     * Parse team matchup string like "Lakers vs Celtics" or "Lakers @ Celtics"
     */
    parseTeamMatchup(teamsString: string) {
     ...
     */
    validateSportsMarket(marketType: string, params: any) {
        switch (marketType) {
            case 'SPORTS_WINNER':
                if (!params.betTeam) {
