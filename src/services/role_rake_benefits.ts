...
export interface RoleRakeBenefit {
  reductionRate: number; // 0-100% rake reduction
...
const roleRakeCache = new Map<string, Map<string, RoleRakeBenefit>>();
...
export class RoleRakeReductionService {
  ...
  static async getBestRakeReduction(
    userId: number,
    guildId: string,
  ...
  private static async getRoleRakeBenefit(
    discordUserId: string,
    guildId: string
  ): Promise<RoleRakeBenefit | null> {
    ...
    try {
      ...
      let bestBenefit: RoleRakeBenefit | null = null;
  ...
  private static async refreshRakeCache(guildId: string): Promise<void> {
    try {
      ...
      const guildCache = new Map<string, RoleRakeBenefit>();
  ...
  private static async getTierRakeBenefit(userId: number): Promise<RoleRakeBenefit | null> {
    try {
  ...
  static async cleanupExpiredReductions(): Promise<number> {
    try {
  ...
  static async getAllActiveReductions(guildId?: string) {
