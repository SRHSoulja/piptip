...
export function validateAchievementData(data: any): ValidationResult {
  const errors: string[] = [];
  const {
    name, description, criteriaType, criteriaData, threshold,
    iconEmoji, badgeColor, category, rarity
  } = data;
...
export function validateManualOperation(data: any): ValidationResult {
  const errors: string[] = [];
...
export function sanitizeDiscordId(input: any): string | null {
