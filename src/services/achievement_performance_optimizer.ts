...
export class ProgressTableOptimizer {
  ...
  static async archiveCompletedProgress(olderThanDays = 30): Promise<number> {
  ...
  static async cleanupStaleProgress(staleAfterDays = 90): Promise<number> {
  ...
  static async ensureProgressTracking(definitionId: number, batchSize = 1000): Promise<number> {
...
export class BatchProcessor {
  ...
  static async batchProcessAchievementChecks(
    definitionIds: number[],
    batchSize = 100,
    maxConcurrent = 3
  ): Promise<{ processed: number; unlocked: number; errors: number }> {
    ...
    try {
      ...
      for (let i = 0; i < users.length; i += batchSize) {
        ...
        for (const defBatch of definitionBatches) {
          const batchPromises = defBatch.map(definitionId =>
            processBatchForDefinition(userBatch, definitionId)
          );
  ...
  static async batchUpdateProgress(
    updates: Array<{ userId: number; definitionId: number; progress: number; data?: any }>
  ): Promise<number> {
...
async function processBatchForDefinition(
  users: Array<{ id: number; discordId: string }>,
  definitionId: number
...
async function calculateProgressSimplified(userId: number, definition: any): Promise<number> {
...
export class DatabaseOptimizer {
  ...
  static async analyzeTableSizes(): Promise<{
  ...
  static async suggestIndexOptimizations(): Promise<string[]> {
    const suggestions: string[] = [];
