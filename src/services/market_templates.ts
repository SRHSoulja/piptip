...
export interface MarketTemplate {
  ...
  resolutionMethod: 'API_AUTO' | 'API_SPORTS' | 'MANUAL_ADMIN';
  requiresDataSource?: boolean;
  requiresThreshold?: boolean;
  adminOnly: boolean;
  examples?: string[];
  allowCustomOutcomes?: boolean;
  minOutcomes?: number;
  maxOutcomes?: number;
  defaultMinBet?: number;
  defaultMaxBet?: number;
  category: 'sports' | 'crypto' | 'custom' | 'special';
}

// API-Compatible Templates (Auto-Resolution)
export const API_COMPATIBLE_TEMPLATES: MarketTemplate[] = [
  {
...
export const ADMIN_SPECIAL_TEMPLATES: MarketTemplate[] = [
  {
...
export class MarketTemplateService {
   ...
   */
  getAllTemplates(isAdmin: boolean = false): MarketTemplate[] {
   ...
   */
  getTemplatesByCategory(category: string, isAdmin: boolean = false): MarketTemplate[] {
   ...
   */
  getTemplate(templateType: string): MarketTemplate | null {
   ...
   */
  canUserUseTemplate(templateType: string, isAdmin: boolean = false): boolean {
   ...
   */
  getApiCompatibleTemplates(): MarketTemplate[] {
   ...
   */
  getManualResolutionTemplates(): MarketTemplate[] {
    ...
    try {
      ...
      [...API_COMPATIBLE_TEMPLATES, ...ADMIN_SPECIAL_TEMPLATES].forEach(t => {
        if (categoryCounts.hasOwnProperty(t.category)) {
          categoryCounts[t.category as keyof typeof categoryCounts]++;
        }
      });
