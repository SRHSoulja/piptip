...
class PerformanceMonitor {
  ...
  // Get metrics summary
  getMetricsSummary(operation?: string): {
    count: number;
    avgDuration: number;
    minDuration: number;
    maxDuration: number;
...
export function endTimer(operation: string, metadata?: Record<string, any>): number {
  return monitor.endTimer(operation, metadata);
}

export function getMetricsSummary(operation?: string) {
...
export function startPerformanceReporting(intervalMinutes: number = 5): NodeJS.Timeout {
  return setInterval(() => {
    console.log("\n📊 Performance Report:");
