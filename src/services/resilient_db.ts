...
export const resilientDb = {
  ...
  async findGroupTipFast(id: number) {
    return withRetry(
      () => prisma.groupTip.findUnique({
        where: { id },
        include: {
      ...
      `findGroupTipFast(${id})`,
      { maxAttempts: 10, baseDelayMs: 100, maxDelayMs: 1000, exponentialBackoff: false } // Ultra-fast for severe DB issues
    );
  },

  async findGroupTipBasic(id: number) {
    return withRetry(
      () => prisma.groupTip.findUnique({
        where: { id },
  ...
  async finalizeGroupTip(groupTipId: number) {
    return withRetry(
      async () => {
      ...
      { maxAttempts: 5, baseDelayMs: 2000 } // More aggressive for critical operations
    );
  },

  async finalizeGroupTipFast(groupTipId: number) {
    return withRetry(
      async () => {
        const { finalizeExpiredGroupTip } = await import("../features/finalizeExpiredGroupTip.js");
        return finalizeExpiredGroupTip(groupTipId);
  ...
  async createGroupTipClaim(groupTipId: number, userId: number) {
    return withRetry(
  ...
  async processGroupTipClaim(groupTipId: number, discordId: string) {
  ...
  async updateGroupTipMessage(client: any, groupTipId: number) {
    return withRetry(
      async () => {
  ...
  async checkUserGroupTipStatus(discordId: string, groupTipId: number) {
    return withRetry(
      async () => {
