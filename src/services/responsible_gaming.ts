...
interface DailyLimits {
  ...
  currentDailyPredictions: number;
...
class ResponsibleGamingService {
  private readonly DEFAULT_DAILY_LOSS_LIMIT = 1000; // Default max daily loss in tokens
  private readonly DEFAULT_DAILY_PREDICTION_LIMIT = 20; // Max predictions per day
  private readonly MIN_AGE_REMINDER_DAYS = 30; // Show age reminder every 30 days

  /**
   * Check if user can place a prediction based on responsible gaming limits
   */
  async canUserPredict(userId: string, amount: number, tokenSymbol: string): Promise<{
    allowed: boolean;
    reason?: string;
    suggestion?: string;
  }> {
   ...
   */
  async checkSelfExclusion(userId: string): Promise<{
    allowed: boolean;
    reason?: string;
    suggestion?: string;
  }> {
   ...
   */
  async selfExclude(userId: string, durationHours?: number): Promise<{
    success: boolean;
    message: string;
  }> {
    try {
   ...
   */
  private async getUserLimits(userId: string): Promise<DailyLimits> {
  ...
  async getUserStatus(userId: string): Promise<{
    ...
    weeklyStats: {
      ...
      potentialLossThisWeek: number;
    };
    lastAgeReminderShown?: Date;
    needsAgeReminder: boolean;
  }> {
    const limits = await this.getUserLimits(userId);
   ...
   */
  async recordAgeReminderShown(userId: string): Promise<void> {
    try {
      const user = await prisma.user.findFirst({
        where: { discordId: userId }
      });
   ...
   */
  generateResponsibleGamingMessage(): string {
    return `
