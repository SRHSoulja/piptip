...
import type { TreasurySafetyReport } from './treasury_safety_monitor.js';
...
class TreasuryWarningSystemService {
  private alertCooldowns = new Map<string, number>(); // Prevent spam alerts
  private readonly COOLDOWN_MINUTES = 60; // Minimum time between same alerts

  /**
   * Process treasury safety report and generate alerts
   */
  async processSafetyReport(report: TreasurySafetyReport): Promise<TreasuryAlert[]> {
   ...
   */
  private shouldGenerateAlert(alertKey: string): boolean {
   ...
   */
  private async storeAlert(alert: TreasuryAlert): Promise<void> {
    try {
   ...
   */
  async sendCriticalAlert(alert: TreasuryAlert): Promise<void> {
   ...
   */
  private async sendDiscordAdminAlert(alert: TreasuryAlert): Promise<void> {
