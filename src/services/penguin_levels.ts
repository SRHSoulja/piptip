...
function getXPBonus(level: number): number {
  ...
  if (level >= 10) return 0.50; // 50% bonus for max level
  if (level >= 9) return 0.40;  // 40% bonus
  if (level >= 8) return 0.35;  // 35% bonus
  if (level >= 7) return 0.30;  // 30% bonus
  if (level >= 6) return 0.25;  // 25% bonus
  if (level >= 5) return 0.20;  // 20% bonus
  if (level >= 4) return 0.15;  // 15% bonus
  if (level >= 3) return 0.10;  // 10% bonus
  if (level >= 2) return 0.05;  // 5% bonus
  return 0; // No bonus for level 1
}

// Daily XP tracking cache
const dailyXpCache = new Map<string, { date: string; total: number; byCategory: Record<string, number> }>();
...
export async function awardXP(
  ...
  const categoryLimit = DAILY_XP_CAPS[category as keyof typeof DAILY_XP_CAPS] || DAILY_XP_CAPS.TOTAL_DAILY;
...
function getXpCategory(source: string): string {
