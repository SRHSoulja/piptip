...
import { predictionMarkets, Market } from "./prediction_markets.js";
...
export class MarketResolverService {
   ...
   */
  async getTokenRankingByVolume(symbol: string, chain?: string): Promise<{ rank: number; success: boolean; error?: string }> {
    try {
      ...
      if (volume > 10000000) rank = 1; // $10M+ volume = top rank
      else if (volume > 5000000) rank = 2;
      else if (volume > 1000000) rank = 3;
      else if (volume > 500000) rank = 5;
      else if (volume > 100000) rank = 10;
      else if (volume > 50000) rank = 20;
   ...
   */
  async resolvePriceMarket(market: Market): Promise<{ outcome: 'YES' | 'NO' | 'CANCEL'; data: MarketResolutionData }> {
    const { marketData } = market;
    const symbol = marketData.symbol || market.tokenSymbol;

    let currentPriceData: TokenPriceData;
   ...
   */
  async resolveRankingMarket(market: Market): Promise<{ outcome: 'YES' | 'NO' | 'CANCEL'; data: MarketResolutionData }> {
    const { marketData } = market;
    const symbol = marketData.symbol || market.tokenSymbol;
   ...
   */
  async resolveMarket(marketId: string): Promise<{ success: boolean; outcome?: 'YES' | 'NO' | 'CANCEL'; error?: string }> {
    try {
   ...
   */
  async resolvePriceRangeMarket(market: Market): Promise<{ outcome: 'YES' | 'NO' | 'CANCEL'; data: MarketResolutionData }> {
    const marketData = market.marketData as any;
   ...
   */
  async resolveExpiredMarkets(): Promise<{ resolved: number; errors: number }> {
    let resolved = 0;
    let errors = 0;
