...
export class AdminPermissionService {
   ...
   */
  async isUserSuperUser(discordId: string): Promise<boolean> {
    try {
      const user = await prisma.user.findUnique({
        where: { discordId },
   ...
   */
  async canUserCreateMarkets(discordId: string): Promise<boolean> {
    try {
      const user = await prisma.user.findUnique({
        where: { discordId },
        select: {
   ...
   */
  async canUserResolveMarkets(discordId: string): Promise<boolean> {
   ...
   */
  async canUserCreateSpecialMarkets(discordId: string): Promise<boolean> {
    return await this.isUserAdmin(discordId);
  }
   ...
   */
  async canUserPromoteUsers(discordId: string): Promise<boolean> {
   ...
   */
  async getUserAdminProfile(discordId: string): Promise<AdminUser | null> {
    try {
      const user = await prisma.user.findUnique({
        where: { discordId },
        select: {
