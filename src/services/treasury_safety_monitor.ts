...
import { getActiveTokens, formatAmount } from './token.js';
import { priceAPI } from './price_api.js';
import type { TokenRow } from './token.js';

interface TreasurySafetyStatus {
  tokenId: number;
  tokenSymbol: string;
  currentBalanceAtomic: bigint;
  currentBalanceHuman: number;
...
interface TreasurySafetyReport {
  timestamp: Date;
  overallStatus: 'safe' | 'warning' | 'critical';
  monitoringEnabled: boolean;
  autoTransferEnabled: boolean;
  coldWalletAddress?: string;
  tokenStatuses: TreasurySafetyStatus[];
...
class TreasurySafetyMonitorService {
   ...
   */
  private async getUSDValue(balanceHuman: number, tokenSymbol: string): Promise<number> {
    try {
   ...
   */
  async checkTreasurySafety(): Promise<TreasurySafetyReport> {
    if (this.isChecking) {
    ...
    try {
      ...
      const tokenStatuses: TreasurySafetyStatus[] = [];
      ...
      for (const balance of treasurySnapshot.tokens) {
        ...
        let excessUSD: number | undefined;
   ...
   */
  async executeAutoTransfer(tokenId: number, excessAmountAtomic: bigint): Promise<{ success: boolean; txHash?: string; error?: string }> {
    const config = await this.getTreasurySafetyConfig();
   ...
   */
  async logTreasuryAction(action: string, metadata: Record<string, any>) {
    try {
   ...
   */
  shouldRunCheck(checkIntervalMins: number): boolean {
...
export type { TreasurySafetyReport, TreasurySafetyStatus };