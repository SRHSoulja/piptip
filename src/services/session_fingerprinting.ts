...
class SessionFingerprintingService {
  ...
  private readonly MAX_ACTIVITIES = 1000; // Keep last 1000 suspicious activities
   ...
   */
  private generateFingerprint(req: Request): SessionFingerprint {
   ...
   */
  async validateFingerprint(sessionId: string, req: Request): Promise<{
    valid: boolean;
    riskLevel: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';
    action: 'ALLOW' | 'WARN' | 'VERIFY' | 'BLOCK';
    changes?: string[];
   ...
   */
  private compareFingerprints(stored: SessionFingerprint, current: SessionFingerprint): string[] {
   ...
   */
  private analyzeRisk(changes: string[], stored: SessionFingerprint, current: SessionFingerprint): {
    riskLevel: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';
    action: 'ALLOW' | 'WARN' | 'VERIFY' | 'BLOCK';
  } {
    let riskScore = 0;
    ...
    const recentSuspiciousChanges = stored.suspiciousChanges.filter(change =>
      change.includes('IP changed') || change.includes('User-Agent changed')
    ).length;
  ...
  fingerprintMiddleware() {
    return async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        (req as any).fingerprintValidation = validation;
   ...
   */
  markAsVerified(sessionId: string): boolean {
  ...
  private cleanup(): void {
    ...
    const oldActivities = this.suspiciousActivities.filter(activity =>
      now - activity.timestamp.getTime() > OLD_THRESHOLD
    );
...
// Utility functions
export function markSessionAsVerified(sessionId: string) {
