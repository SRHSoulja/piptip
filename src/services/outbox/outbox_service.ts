...
export class OutboxService {
  ...
  private async sendToDiscord(message: any): Promise<void> {
    ...
    if (payload.embeds && payload.embeds.length > 0) {
      messageOptions.embeds = payload.embeds.map((embedData: any) =>
    ...
    if (payload.components && payload.components.length > 0) {
      messageOptions.components = payload.components.map((componentData: any) => {
        if (componentData.components) {
          return new ActionRowBuilder().addComponents(
            componentData.components.map((comp: any) => {
   ...
   */
  private async handleMessageFailure(message: any, error: Error): Promise<void> {
   ...
   */
  private trackRateLimit(guildId: string): void {
    const now = Date.now();
    const tracker = this.rateLimitTracker.get(guildId);
   ...
   */
  private generateMessageHash(message: OutboxMessage): string {
    const content = {
      aggregateId: message.aggregateId,
      eventType: message.eventType,
      channelId: message.channelId,
