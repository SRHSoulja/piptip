...
interface RefreshToken {
  ...
  lastUsed: Date;
  revoked: boolean;
  fingerprint: string; // Browser/client fingerprint for security
}
...
class JWTAuthService {
   ...
   */
  async generateTokenPair(sessionId: string, adminId: string, permissions: string[], req: Request): Promise<TokenPair> {
   ...
   */
  async refreshAccessToken(refreshToken: string, req: Request): Promise<{
    success: boolean;
    tokenPair?: TokenPair;
    error?: string;
  }> {
   ...
   */
  jwtMiddleware(requiredPermissions: string[] = []) {
    return async (req: Request, res: Response, next: NextFunction) => {
      ...
      if (requiredPermissions.length > 0) {
        const hasPermission = requiredPermissions.some(perm =>
          verification.payload!.permissions.includes(perm)
        );
      ...
      (req as any).jwtPayload = verification.payload;
      (req as any).adminId = verification.payload!.adminId;
      (req as any).sessionId = verification.payload!.sessionId;
      next();
    };
  }
   ...
   */
  revokeAllTokensForSession(sessionId: string): number {
    let revokedCount = 0;
...
export const jwtMiddleware = (permissions: string[] = []) => jwtAuth.jwtMiddleware(permissions);
...
// Utility functions
export async function generateTokenPair(sessionId: string, adminId: string, permissions: string[], req: Request) {
  return jwtAuth.generateTokenPair(sessionId, adminId, permissions, req);
}

export async function refreshAccessToken(refreshToken: string, req: Request) {
...
export function revokeAllTokensForSession(sessionId: string) {
