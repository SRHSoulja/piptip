// src/services/command_wrapper.ts - Transaction logging wrapper for commands
import { Prisma } from "@prisma/client";
import { prisma } from "./db.js";
import { logCompleteTransaction, type BalanceChange } from "./tx_logger.js";
...
export async function withTransactionLogging<T>(
  ...
  options?: {
    guildId?: string;
    trackBalanceChanges?: boolean; // Set to true if the command modifies balances
    metadata?: any;
  }
): Promise<T> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    let beforeBalances: Map<string, bigint> = new Map();
 ...
 */
export async function withCommandLogging<T>(
  operation: string,
  userId: string,
  handler: () => Promise<T>,
  options?: {
 ...
 */
export async function withButtonTransactionLogging<T>(
  buttonId: string,
  userId: string,
  handler: (tx: Tx) => Promise<T>,
  options?: {
