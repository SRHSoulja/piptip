...
interface SecureKey {
  ...
  rotateKey(newPrivateKey: string): boolean;
  clearMemory(): void;
}

class SecureKeyManager implements SecureKey {
  private encryptedKey: string | null = null;
  private keyDerivation: Buffer | null = null;
  private lastAccessed: number = 0;
  private accessCount: number = 0;
  private readonly maxMemoryTime = 5 * 60 * 1000; // 5 minutes
  private readonly maxAccessCount = 1000; // Prevent excessive access
...
class SecureCredentialManager {
  private credentialCache = new Map<string, { encrypted: string; lastAccessed: number; accessCount: number }>();
  private derivationKey: Buffer | null = null;
  private readonly maxMemoryTime = 5 * 60 * 1000; // 5 minutes
  private readonly maxAccessCount = 1000;
  ...
  private initializeDerivationKey(): void {
    const systemSalt = (process.env.NODE_ENV || 'development') + process.pid + Math.random();
  ...
  private decryptCredential(encryptedData: string): string {
    if (!this.derivationKey) {
      throw new Error('SECURITY: Credential manager not initialized');
    }
  ...
  getSecureCredential(envVarName: string): string {
  ...
  private validateCredential(envVarName: string, value: string): void {
  ...
  private cleanupStaleCredentials(): void {
    const now = Date.now();
  ...
  getCredentialStats(): Record<string, { lastAccessed: number; accessCount: number; timeSinceAccess: number }> {
...
export function getSecureTreasuryPrivateKey(): string {
  return secureKeyManager.getPrivateKey();
}

export function validateTreasuryKeyIntegrity(): boolean {
  return secureKeyManager.validateKeyIntegrity();
}

export function rotateTreasuryKey(newPrivateKey: string): boolean {
  return secureKeyManager.rotateKey(newPrivateKey);
}

export function clearTreasuryKeyFromMemory(): void {
...
export function getSecureCredential(envVarName: string): string {
