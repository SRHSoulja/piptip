...
export interface TPIPPurchaseResult {
  success: boolean;
  tpipAmount?: bigint;
...
export interface TournamentTPIPState {
  userId: number;
  tpipBalance: bigint;
  tournamentId?: string;
  isActive: boolean;
...
export async function purchaseTPIP(params: {
  userId: number;
  discordId: string;
  tokenId: number; // Abstract token being used (ABSTR, etc.)
  amount: bigint; // Amount of Abstract token to spend
  guildId?: string;
}): Promise<TPIPPurchaseResult> {
 ...
 */
export async function getTPIPBalance(userId: number): Promise<bigint> {
 ...
 */
export async function getTournamentTPIPState(userId: number): Promise<TournamentTPIPState> {
