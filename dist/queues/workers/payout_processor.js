...
export class PayoutProcessor {
    worker;
    constructor() {
        this.worker = new Worker('payouts', this.processJob.bind(this), {
            connection: redis,
    ...
    async processMarketPayouts(jobData, job) {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            await redis.setex(`payout:processed:${jobData.idempotencyKey}`, 86400, // 24 hours
            JSON.stringify({
                processed: true,
                timestamp: Date.now(),
    ...
    async escalatePayoutFailure(job, error) {
        ...
        await redis.setex(`market:locked:${job.data.marketId}`, 3600, // 1 hour
        JSON.stringify({
    ...
    async logPayoutError(job, error) {
        ...
        await redis.setex(`payout:error:${job.data.marketId}:${job.id}`, 172800, // 48 hours for financial errors
