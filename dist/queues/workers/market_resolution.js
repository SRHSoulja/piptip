...
export class MarketResolutionWorker {
    worker;
    constructor() {
        this.worker = new Worker('market-resolution', this.processJob.bind(this), {
            connection: redis,
    ...
    async checkExistingResolution(marketId, jobData) {
        ...
        await redis.setex(`market:resolving:${marketId}`, 300, // 5 minutes
        JSON.stringify({
            jobId: jobData.id,
            timestamp: Date.now(),
    ...
    async queuePayoutProcessing(jobData) {
        ...
        await payoutQueue.add('process-payouts', payoutJobData, {
    ...
    async logResolutionError(job, error) {
        ...
        await redis.setex(`market:resolution:error:${job.data.marketId}:${job.id}`, 86400, // 24 hours
