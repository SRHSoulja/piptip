...
export const metricsMiddleware = (req, res, next) => {
    ...
    // Track request start
    metrics.requestRate.observe({ endpoint: route, method: req.method }, 1);
    ...
    res.on('finish', () => {
        ...
        try {
            ...
            metrics.requestRate.observe({ endpoint: route, method: req.method }, duration);
            // Log slow requests
            ...
            if (statusClass >= 4) {
                recordError(`http_${statusClass}xx`, statusClass === 5 ? 'high' : 'medium', 'http');
            }
        }
        catch (error) {
 ...
 */
export const databaseMetricsWrapper = (operation, table, queryFn) => {
    const startTime = Date.now();
    return queryFn()
        .then(result => {
 ...
 */
export const redisMetricsWrapper = (operation, operationFn) => {
    const startTime = Date.now();
    return operationFn()
        .then(result => {
        const duration = Date.now() - startTime;
 ...
 */
export const trackDatabaseConnections = (count) => {
    metrics.databaseConnections.set(count);
};
/**
 * Utility to update active market counts
 */
export const updateActiveMarketCount = async () => {
    try {
 ...
 */
export const startBusinessMetricsCollection = () => {
    ...
    setInterval(async () => {
        try {
            ...
            metrics.activeUsers.set({ period: '7d', guild_id: 'all' }, activeUsers7d.length);
        }
        catch (error) {
