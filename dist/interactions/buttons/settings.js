import { EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, ChannelSelectMenuBuilder, ChannelType } from "discord.js";
import { getGuildSettings, updateGuildSettings } from "../../services/channel_manager.js";
export async function handleSettingsMode(i) {
...
export async function handleSelectMode(i) {
...
export async function handleSettingsChannels(i) {
...
export async function handleAddAllowed(i) {
    const channelSelector = new ActionRowBuilder()
...
export async function handleAddBlocked(i) {
    const channelSelector = new ActionRowBuilder()
        .addComponents(new ChannelSelectMenuBuilder()
...
export async function handleSelectAllowed(i) {
    const settings = await getGuildSettings(i.guildId);
...
export async function handleSelectBlocked(i) {
    const settings = await getGuildSettings(i.guildId);
    const newChannels = i.values;
...
export async function handleFeatureChannels(i) {
    const embed = new EmbedBuilder()
...
// Preset handlers
export async function handleSetupEverywhere(i) {
...
export async function handleSetupGaming(i) {
    const guild = i.guild;
    const gameChannels = [];
    const tipChannels = [];
...
export async function handleSetupStrict(i) {
    const guild = i.guild;
    let pipChannel = guild.channels.cache.find(c => c.isTextBased() && c.name.toLowerCase().includes('piptip'));
    if (!pipChannel) {
        // Look for bot/command channels
        pipChannel = guild.channels.cache.find(c => c.isTextBased() && (c.name.toLowerCase().includes('bot') ||
