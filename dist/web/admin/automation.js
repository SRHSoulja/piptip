...
const automationAdminRouter = Router();
automationAdminRouter.get("/status", async (req, res) => {
  try {
...
automationAdminRouter.get("/config", (req, res) => {
  try {
...
automationAdminRouter.post("/config", async (req, res) => {
  try {
    ...
    const needsRestart = updates.enabled !== void 0 || updates.schedule !== void 0;
    if (needsRestart) {
...
automationAdminRouter.post("/trigger", async (req, res) => {
  try {
...
automationAdminRouter.post("/start", (req, res) => {
  try {
...
automationAdminRouter.post("/stop", (req, res) => {
  try {
...
automationAdminRouter.get("/logs", async (req, res) => {
  try {
    const {
      type,
      success,
    ...
    const since = new Date(Date.now() - daysNum * 24 * 60 * 60 * 1e3);
    const where = {
      createdAt: { gte: since }
    };
    if (type) {
    ...
    res.json({
      ...
      logs: logs.map((log) => ({
        ...
        engagementMetrics: log.engagementMetrics ? typeof log.engagementMetrics === "string" ? JSON.parse(log.engagementMetrics) : log.engagementMetrics : null
      })),
      pagination: {
        total,
        limit: limitNum,
...
automationAdminRouter.get("/analytics", async (req, res) => {
  try {
    ...
    const performanceByType = {
      crypto: {
        ...
        successful: cryptoLogs.filter((log) => log.success).length,
        successRate: cryptoLogs.length > 0 ? cryptoLogs.filter((log) => log.success).length / cryptoLogs.length * 100 : 0
      },
      sports: {
        total: sportsLogs.length,
        successful: sportsLogs.filter((log) => log.success).length,
        successRate: sportsLogs.length > 0 ? sportsLogs.filter((log) => log.success).length / sportsLogs.length * 100 : 0
      }
    };
    const errors = logs.filter((log) => !log.success && log.error).map((log) => log.error);
    const errorCounts = {};
    errors.forEach((error) => {
      errorCounts[error] = (errorCounts[error] || 0) + 1;
    });
    const topErrors = Object.entries(errorCounts).sort(([, a], [, b]) => b - a).slice(0, 5).map(([error, count]) => ({ error, count }));
    res.json({
      success: true,
      analytics: {
        period: {
        ...
        dailyBreakdown: Object.entries(dailyStats).sort(([a], [b]) => a.localeCompare(b)).map(([date, stats]) => ({ date, ...stats })),
        performanceByType,
        topErrors
      }
    });
...
automationAdminRouter.delete("/logs", async (req, res) => {
  try {
