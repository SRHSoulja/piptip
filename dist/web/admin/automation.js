...
automationAdminRouter.get("/logs", async (req, res) => {
    try {
        const { type, success, limit = "50", offset = "0", days = "7" } = req.query;
        ...
        res.json({
            ...
            logs: logs.map(log => ({
                ...
                engagementMetrics: log.engagementMetrics ?
                    (typeof log.engagementMetrics === 'string' ? JSON.parse(log.engagementMetrics) : log.engagementMetrics)
                    : null
            })),
            pagination: {
                total,
...
automationAdminRouter.get("/analytics", async (req, res) => {
    try {
        ...
        logs.forEach(log => {
            ...
            if (log.type === 'crypto')
                dailyStats[date].crypto++;
            if (log.type === 'sports')
                dailyStats[date].sports++;
