...
const emergencyRateLimit = rateLimit({
  ...
  // 5 minutes
  max: 10,
  // Very restrictive for emergency operations
  message: {
...
router.post("/emergency/:action", async (req, res) => {
  try {
    ...
    console.log(`\u{1F6A8} EMERGENCY ${action.toUpperCase()}: ${affected} achievements affected${category ? ` (category: ${category})` : ""} - Reason: ${reason || "Not specified"}`);
    res.json({
      success: true,
      action,
      affected,
...
router.post("/bulk-threshold", async (req, res) => {
  try {
    ...
    let updateData = { version: { increment: 1 } };
    if (newThreshold !== void 0) {
      ...
      const updates = definitions.map(
        (def) => prisma.achievementDefinition.update({
          where: { id: def.id },
          data: {
  ...
  try {
    ...
    res.json({
      ...
      completionAnalysis: {
        ...
        averageCompletionRate: completionRates.length > 0 ? (completionRates.reduce((sum, item) => sum + parseFloat(item.completionRate), 0) / completionRates.length).toFixed(1) : "0"
      },
...
var quick_wins_default = router;
export {
  quick_wins_default as default
};
