...
router.post('/bulk-threshold', async (req, res) => {
    try {
        ...
        if (newThreshold !== undefined) {
            ...
            const updates = definitions.map(def => prisma.achievementDefinition.update({
                where: { id: def.id },
                data: {
...
router.get('/stats/overview', async (req, res) => {
    try {
        const [totalDefinitions, enabledDefinitions, totalUnlocks, totalProgress, categoryBreakdown, rarityBreakdown, recentActivity] = await Promise.all([
        ...
        const completionRates = await Promise.all((await prisma.achievementDefinition.findMany({
            where: { isEnabled: true },
