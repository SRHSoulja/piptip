// src/web/admin/role_tax_management.ts - Admin interface for Discord role tax exemptions
...
router.get('/', async (req, res) => {
    try {
        ...
        const exemptions = await RoleTaxBenefitService.getAllActiveExemptions(guildId);
        // Enrich with Discord role names
        const enrichedExemptions = await Promise.all(exemptions.map(async (exemption) => {
            try {
...
// POST /admin/role-tax - Create new role tax exemption
router.post('/', async (req, res) => {
    try {
        const { roleId, guildId, exemptionRate, duration, label, notes } = req.body;
        // Validation
        const errors = [];
...
// DELETE /admin/role-tax/:id - Remove role tax exemption
...
// PUT /admin/role-tax/:id - Update existing exemption
router.put('/:id', async (req, res) => {
    try {
        ...
        const updated = await prisma.roleTaxExemption.update({
            where: { id: exemptionId },
            data: {
                ...(exemptionRate !== undefined && { exemptionRate: Number(exemptionRate) }),
                ...(duration !== undefined && { duration: Number(duration), expiresAt }),
...
// GET /admin/role-tax/preview/:guildId/:roleId - Preview role before creating exemption
...
// GET /admin/role-tax/stats - Get usage statistics
...
// GET /admin/role-tax/analytics/:guildId/:roleId - Partnership engagement report
router.get('/analytics/:guildId/:roleId', async (req, res) => {
    try {
        const { guildId, roleId } = req.params;
        const { days = '30' } = req.query;
        const report = await RoleAnalyticsService.getPartnershipReport(roleId, guildId, parseInt(days));
        res.json({
            success: true,
            report,
            period: `${days} days`
...
// POST /admin/role-tax/analytics/generate - Manually trigger analytics generation
router.post('/analytics/generate', async (req, res) => {
    try {
        ...
        if (roleId && guildId) {
            ...
            await RoleAnalyticsService.generateDailyAnalytics(roleId, guildId, date ? new Date(date) : new Date());
            processed = 1;
        }
        else {
            // Generate for all roles
            processed = await RoleAnalyticsService.generateAllRoleAnalytics(date ? new Date(date) : new Date());
        }
        res.json({
            success: true,
