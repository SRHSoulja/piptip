  ...
  try {
    ...
    transactions.forEach((tx) => {
      ...
      if (tx.type === "TIP" && direction === "incoming" && counterpart === "system") {
        const matchingGroupTip = groupTips.created.find(
          (gt) => gt.status === "REFUNDED" && Math.abs(new Date(gt.createdAt).getTime() - new Date(tx.createdAt).getTime()) < 6e4
          // Within 1 minute
        );
        if (matchingGroupTip) {
          ...
          const finalizedGroupTip = groupTips.created.find(
            (gt) => gt.status === "FINALIZED" && Math.abs(new Date(gt.createdAt).getTime() - new Date(tx.createdAt).getTime()) < 3e5
            // Within 5 minutes
          );
...
`;
    ...
    const filename = `user_${discordId}_activity_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv`;
...
transactionsRouter.get("/transactions/export/guild/:guildId", async (req, res) => {
  try {
    ...
    const filename = `guild_${guildId}_activity_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv`;
    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename="${filename}"`);
    res.send(csv);
  } catch (error) {
...
transactionsRouter.delete("/transactions/:id", async (req, res) => {
  try {
    ...
    console.log(`\u{1F5D1}\uFE0F Admin deleted transaction: ID ${transactionId}, Type: ${transaction.type}, Amount: ${transaction.amount}`);
    res.json({
      ok: true,
