...
async function getDeadLetterDashboard(req, res) {
  try {
...
async function getDeadLetterJob(req, res) {
  try {
    const { id } = req.params;
...
async function dismissDeadLetterJob(req, res) {
  try {
    const { id } = req.params;
...
async function getDeadLetterStats(req, res) {
  try {
    const stats = await deadLetterWorker.getDeadLetterStats();
...
async function bulkRetryDeadLetterJobs(req, res) {
  try {
...
async function cleanupDeadLetterJobs(req, res) {
  try {
    ...
    console.log(`\u{1F9F9} Cleaned up ${result.count} old dead letter jobs (older than ${olderThanDays} days) by admin ${adminUserId}`);
    res.json({
      success: true,
