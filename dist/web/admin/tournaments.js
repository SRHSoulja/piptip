...
import { getTournamentConfig, setTournamentConfig, distributeTournamentPrizes, calculateDynamicPrizePool, displayTournamentPrizePool, getTournamentLeaderboard } from "../../services/tournaments.js";
...
tournamentsRouter.post("/tournaments", async (req, res) => {
    try {
        const { id, name, description, type = 'LEADERBOARD', entryFeeUSD, maxParticipants, prizeTokens, prizeDistribution, registrationStart, registrationEnd, tournamentStart, tournamentEnd } = req.body;
...
tournamentsRouter.post("/tournaments/:id/distribute-prizes", async (req, res) => {
    try {
        ...
        if (result.success) {
            const updatedConfig = { ...config, status: 'COMPLETED', updatedAt: new Date() };
