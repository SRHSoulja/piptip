...
router.get("/", viewOnlyAdminMiddleware(), async (req, res) => {
  try {
    ...
    if (enabled !== void 0) where.isEnabled = enabled === "true";
    ...
    const enrichedDefinitions = await Promise.all(
      definitions.map(async (def) => {
        ...
        const completionRate = totalUsers > 0 ? (def._count.unlockedAchievements / totalUsers * 100).toFixed(1) : "0.0";
        return {
          ...def,
          stats: {
  ...
  try {
    ...
    const definition = await prisma.achievementDefinition.create({
      data: {
        ...
        tier: Number(tier),
        createdBy: "admin"
        // Could be extracted from JWT in production
      }
    });
