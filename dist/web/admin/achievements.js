...
import { validateAchievementData, validateBulkOperation } from '../../services/input_validation.js';
...
router.get('/', viewOnlyAdminMiddleware(), async (req, res) => {
    try {
        ...
        const enrichedDefinitions = await Promise.all(definitions.map(async (def) => {
...
router.post('/', basicAdminMiddleware(), async (req, res) => {
    try {
        ...
        const { name, description, category, criteriaType, criteriaData, threshold, iconEmoji, badgeColor = '#FFD700', rarity = 'common', isEnabled = true, isVisible = true, isRepeatable = false, cooldownHours, startDate, endDate, sortOrder = 0, tier = 1 } = req.body;
        // Validation
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (updateData.threshold)
            updateData.threshold = Number(updateData.threshold);
        if (updateData.cooldownHours)
            updateData.cooldownHours = Number(updateData.cooldownHours);
        if (updateData.sortOrder)
            updateData.sortOrder = Number(updateData.sortOrder);
        if (updateData.tier)
            updateData.tier = Number(updateData.tier);
        // Convert date fields
...
router.get('/analytics/overview', async (req, res) => {
    try {
        ...
        const [totalDefinitions, activeDefinitions, totalUnlocks, totalUsers, categoryStats, recentUnlocks, topAchievements] = await Promise.all([
        ...
        const topAchievementsWithNames = await Promise.all(topAchievements.map(async (item) => {
            const definition = await prisma.achievementDefinition.findUnique({
...
async function getAchievementAnalytics(definitionId) {
