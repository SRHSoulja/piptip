    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error("Finalization timeout")), 3000))
            ]);
...
async function updateDiscordMessageResilient(client, tipId) {
    try {
...
async function fallbackImmediateUpdate(client, tipId) {
    try {
        ...
        await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error("Fallback update timeout")), 5000))
        ]);
...
export async function scheduleGroupTipExpiry(client, tipId) {
    ...
    try {
        const row = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error("Schedule query timeout")), 1000))
        ]);
...
export async function restoreGroupTipExpiryTimers(client) {
    try {
        ...
        const overdue = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error("Overdue query timeout")), 2000))
        ]);
        ...
        const upcoming = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error("Upcoming query timeout")), 2000))
        ]);
