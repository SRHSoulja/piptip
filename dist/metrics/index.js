...
const metrics = {
  ...
  redisOperationDuration: new Histogram({
    ...
    buckets: [1e-3, 2e-3, 5e-3, 0.01, 0.02, 0.05, 0.1]
  }),
  // === ERROR TRACKING ===
  errors: new Counter({
...
function recordJobProcessing(queueName, jobType, status, duration) {
  metrics.jobsProcessed.inc({ queue_name: queueName, status });
  metrics.jobProcessingDuration.observe({ queue_name: queueName, job_type: jobType }, duration / 1e3);
}
function recordDiscordInteraction(command, status, duration, guildId) {
  metrics.discordInteractions.inc({
    command,
    status,
    guild_id: guildId || "dm"
  });
  metrics.discordInteractionDuration.observe({ command }, duration / 1e3);
}
...
function recordMarketActivity(type, data) {
  if (type === "created") {
...
function recordBet(outcome, amount, tokenSymbol, guildId) {
...
function startMetricsCollection() {
  setInterval(updateSystemHealth, 3e4);
  setInterval(async () => {
    try {
  ...
  console.log("\u{1F4CA} Prometheus metrics collection started");
}
console.log("\u{1F680} Prometheus metrics system initialized");
export {
  metrics,
  recordBalanceDrift,
  recordBet,
