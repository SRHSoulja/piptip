...
const PORT = Number(process.env.PORT || (process.env.REPLIT_DB_URL ? 5e3 : 3e3));
const app = express();
...
const globalLimiter = rateLimit({
  ...
  skip: (req) => {
    return req.path.startsWith("/health") || req.path.startsWith("/internal") || req.path === "/favicon.ico";
  },
...
const corsOptions = cors({
  origin: (origin, callback) => {
    ...
    if (allowedOrigins.includes(origin)) {
      ...
      console.warn(`\u{1F6AB} CORS: Blocked request from origin: ${origin}`);
...
<head>
  ...
  <style>
    ...
    .status::before {
      content: '${botStatus === "Online" ? "\u{1F7E2}" : "\u{1F7E1}"}'; margin-right: 8px;
    }
...
<body>
  <div class="container">
    ${isReplit ? '<div class="replit-badge">\u{1F680} Running on Replit</div>' : ""}
    <h1>\u{1F427} PIPTip</h1>
...
function withAutoAck(fn) {
  return async (i) => {
    ...
    const skipAutoDefer = isButtonInteraction(i) && (i.customId.includes("grouptip:add") || i.customId.includes("_add") || i.customId.includes("grouptip:confirm") || i.customId.includes("grouptip:cancel"));
...
If you believe this is an error, please contact the administrators.`,
  ...
  if ("isChatInputCommand" in i && i.isChatInputCommand()) {
    try {
      ...
      processPendingDiscordUpdates(bot).catch(() => {
      });
    } catch (error) {
    }
    switch (i.commandName) {
      case "pip_withdraw":
        return withAutoChannelCheck(i, pipWithdraw);
      case "pip_profile":
        return withAutoChannelCheck(i, pipProfile);
      case "pip_deposit":
        return withAutoChannelCheck(i, pipDeposit);
      case "pip_game":
        return withAutoChannelCheck(i, pipGame);
      case "pip_link":
        return withAutoChannelCheck(i, pipLink);
      case "pip_tip":
        return withAutoChannelCheck(i, pipTip);
      case "pip_help":
        return withAutoChannelCheck(i, pipHelp);
      case "pip_stats":
        return withAutoChannelCheck(i, pipStats);
      case "pip_bio":
        return withAutoChannelCheck(i, pipBio);
      case "pip_pengubook":
        return withAutoChannelCheck(i, pipPenguBook);
      case "pip_achievements":
        return withAutoChannelCheck(i, pipAchievements);
      case "pip_leaderboard":
        return withAutoChannelCheck(i, pipLeaderboard);
      case "pip_apply":
        return withAutoChannelCheck(i, pipApply);
      case "pip_settings":
        return withAutoChannelCheck(i, pipSettings);
      case "pip_safety":
        return withAutoChannelCheck(i, pipSafety);
      case "pip_balance":
        return withAutoChannelCheck(i, pipBalance);
      case "pip_daily":
        return withAutoChannelCheck(i, pipDaily);
      case "pip_buy_chips":
        return withAutoChannelCheck(i, pipBuyChips);
      case "pip_automation_status":
        return withAutoChannelCheck(i, pipAutomationStatus);
      case "pip_referral":
        return withAutoChannelCheck(i, pipReferral);
      default:
...
async function main() {
  try {
    ...
    try {
      ...
      console.log("\u{1F50D} Session fingerprinting middleware enabled");
    } catch (error) {
    ...
    try {
      ...
      console.log("\u{1F6E1}\uFE0F Security dashboard routes configured");
    } catch (error) {
    ...
    try {
      ...
      console.log("\u{1F6A8} Incident notification system initialized");
    } catch (error) {
    ...
    const shutdown = async () => {
      ...
      try {
        ...
        console.log("\u{1F6D1} Discord rate limiter shutdown");
      } catch (error) {
      ...
      try {
        ...
        console.log("\u{1F6D1} Resilient Discord update service shutdown");
      } catch (error) {
      ...
      try {
        ...
        console.log("\u{1F6D1} Tier management services stopped");
      } catch (error) {
        console.error("Error stopping tier management services:", error);
      }
      try {
        marketAutomationScheduler.stop();
        console.log("\u{1F6D1} Market automation scheduler stopped");
      } catch (error) {
