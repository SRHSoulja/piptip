...
export function matchOfferEmbed(challengerTag, wagerText, ad) {
    const e = new EmbedBuilder()
        ...
        .setDescription(`🐧 **${challengerTag}** is looking for a worthy opponent!\n\n` +
...
export function matchResultEmbed(opts) {
    ...
    e.addFields({ name: "🥊🐧 Challenger", value: challengerValue, inline: true }, { name: "⚔️🐧 Opponent", value: joinerValue, inline: true }, { name: "💥 Battle Outcome", value: `${challengerEmoji} **VS** ${joinerEmoji}`, inline: true });
    // Financial details with better formatting
    if (opts.payoutText || opts.rakeText || opts.potText) {
        const financialDetails = [];
        if (opts.potText)
            financialDetails.push(`🐟 **Total Fish Pool:** ${opts.potText}`);
        if (opts.payoutText)
            financialDetails.push(`🎁 **Victor's Bounty:** ${opts.payoutText}`);
        if (opts.rakeText)
            financialDetails.push(`🏛️ **Colony Tax:** ${opts.rakeText}`);
        e.addFields({
            name: "💸 Fish Economics",
...
export function groupTipEmbed(data) {
    ...
    if (data.isExpired)
        e.setColor(0x999999); // Gray for expired
    return e;
}
