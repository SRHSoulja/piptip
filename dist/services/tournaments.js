 ...
 */
export function calculatePrizeDistribution(config, totalParticipants) {
    const results = [];
 ...
 */
export async function distributeTournamentPrizes(tournamentId, leaderboard) {
    ...
    try {
        await prisma.$transaction(async (tx) => {
            for (const winner of leaderboard) {
                ...
                if (!prizeInfo || prizeInfo.prizes.length === 0)
                    continue;
                try {
 ...
 */
export async function getTournamentLeaderboard(tournamentId, limit = 50) {
    try {
 ...
 */
export async function displayTournamentPrizePool(tournamentId) {
    const config = await getTournamentConfig(tournamentId);
    if (!config)
        return null;
    const breakdown = {};
    ...
    for (const [positionKey, percentage] of Object.entries(config.prizeDistribution)) {
        ...
        if (positionKey.includes('-')) {
            ...
            const perPersonPercentage = percentage / rangeSize;
            breakdown[positionLabel] = config.prizeTokens.map(token => `${(token.amount * perPersonPercentage / 100).toFixed(2)} ${token.tokenSymbol}`);
        }
        else {
            breakdown[positionLabel] = config.prizeTokens.map(token => `${(token.amount * percentage / 100).toFixed(2)} ${token.tokenSymbol}`);
        }
    }
    return {
