...
class AnomalyDetectionService {
    ...
    anomalyAlerts = [];
    cleanupInterval;
    MAX_ALERTS = 5000; // Keep last 5000 alerts
...
export async function analyzeUserBehavior(userId, req, activityType, metadata = {}) {
...
export function resolveAnomalyAlert(alertId, falsePositive = false) {
    return anomalyDetection.resolveAlert(alertId, falsePositive);
}
export function getUserBehaviorProfile(userId) {
    return anomalyDetection.getUserProfile(userId);
}
export function resetUserBehaviorProfile(userId) {
