...
export async function updateStreak(discordId, won) {
    try {
...
export async function getStreakStats(discordId) {
    try {
        return await cacheWithMetrics(CacheKeys.userStreak(discordId), async () => {
...
export async function getUserAchievements(discordId) {
    try {
        return await cacheWithMetrics(CacheKeys.userAchievements(discordId), async () => {
...
export async function getStreakLeaderboard(limit = 10) {
    try {
        const cachedResult = await cacheWithMetrics(CacheKeys.leaderboard('streaks'), async () => {
...
export function formatStreakText(currentWins, longestWins) {
    if (currentWins === 0) {
...
function getStreakEmoji(wins) {
    if (wins >= 100)
        return "🐧💎"; // Diamond Emperor Penguin
    if (wins >= 50)
        return "🐧👑"; // Royal Penguin
    if (wins >= 25)
        return "🐧⚡"; // Lightning Penguin
    if (wins >= 15)
        return "🔥🐧"; // Fire Penguin
    if (wins >= 10)
        return "🌟🐧"; // Star Penguin
    if (wins >= 5)
        return "✨🐧"; // Sparkling Penguin
    if (wins >= 3)
        return "🎯🐧"; // Sharp Penguin
    return "🐧"; // Basic Penguin
}
// Create achievement (reusable utility) - MOVED TO BOTTOM
// Get achievement badge text with penguin theming
export function formatAchievementBadge(achievement) {
...
export async function checkTipAchievements(fromUserId, tipCount, tipAmount) {
    const achievements = [];
    try {
        ...
        const existingMap = new Map(existingAchievements.map((a) => [`${a.type}_${a.level}`, true]));
...
export async function checkDepositAchievements(userId, depositAmount) {
    const achievements = [];
    try {
...
export async function checkEngagementAchievements(userId) {
    const achievements = [];
    try {
