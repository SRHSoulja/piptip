...
function getNetworkDisplayName() {
  ...
  const emoji = config.network === "mainnet" ? "\u{1F680}" : "\u{1F9EA}";
...
function getRegistryContractAddress() {
  const network = getNetworkType();
  if (network === "testnet") {
    const address2 = process.env.TESTNET_REGISTRY_CONTRACT_ADDRESS;
    if (!address2) {
...
try {
  ...
  console.log(`\u{1F4E1} RPC URL: ${config.rpcUrl.replace(/\/v2\/.*/, "/v2/***")}`);
