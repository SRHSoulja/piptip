 ...
 */
function validateNetworkEnv(network) {
    const missingVars = [];
    if (network === 'mainnet') {
        if (!process.env.MAINNET_RPC_URL)
            missingVars.push('MAINNET_RPC_URL');
        if (!process.env.MAINNET_CHAIN_ID)
            missingVars.push('MAINNET_CHAIN_ID');
    }
    else {
        if (!process.env.TESTNET_RPC_URL)
            missingVars.push('TESTNET_RPC_URL');
        if (!process.env.TESTNET_CHAIN_ID)
            missingVars.push('TESTNET_CHAIN_ID');
    }
 ...
 */
export function getRegistryContractAddress() {
    const network = getNetworkType();
    if (network === 'testnet') {
 ...
 */
export function requireMainnetConfirmation(confirmationFlag) {
    if (isMainnet() && !confirmationFlag) {
        throw new Error('🚨 MAINNET OPERATION BLOCKED!\n' +
 ...
 */
export function validateMainnetReadiness() {
    const issues = [];
 ...
 */
let _cachedRpcUrl;
export function getLegacyAbstractRpcUrl() {
    if (!_cachedRpcUrl) {
 ...
 */
let _cachedChainId;
export function getLegacyAbstractChainId() {
    if (!_cachedChainId) {
