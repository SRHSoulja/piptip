...
class AchievementJobQueue {
  ...
  getStatus() {
    return {
      ...
      totalCompleted: 0
      // Could track this with a counter
    };
  }
...
function queueTipAchievements(userId, discordId, tipCount, tipAmount) {
...
function queueDepositAchievements(userId, discordId, depositAmount) {
  return achievementQueue.enqueue({
    type: "deposit",
    userId,
    discordId,
    data: { depositAmount },
    priority: 2
    // Higher priority for deposits
  });
}
function queueMatchAchievements(userId, discordId, won) {
  return achievementQueue.enqueue({
    type: "match",
    userId,
    discordId,
    data: { won },
    priority: 3
    // Highest priority for matches (streaks are time-sensitive)
  });
}
function queueEngagementAchievements(userId, discordId) {
  return achievementQueue.enqueue({
    type: "engagement",
    userId,
    discordId,
...
function achievementQueueHealthCheck() {
  const status = achievementQueue.getStatus();
  return status.queueLength < 100;
}
export {
  achievementQueueHealthCheck,
  clearAchievementQueue,
  getAchievementQueueStatus,
  getUserAchievementData,
  queueDepositAchievements,
  queueEngagementAchievements,
  queueMatchAchievements,
  queueTipAchievements
};
//# sourceMappingURL=achievement_queue.js.map
