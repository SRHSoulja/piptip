...
class MerklePublisher {
    ...
    buildMerkleTreeFromBalanceMap(balanceMap, source) {
        ...
        for (const [address, amount] of balanceMap) {
            const leaf = solidityPackedKeccak256(['address', 'uint256'], [address, amount]);
            leaves.push({
                address,
        ...
        const merkleRoot = this.buildMerkleRoot(leaves.map(l => solidityPackedKeccak256(['address', 'uint256'], [l.address, l.amount])));
    ...
    async publishSnapshot(privateKey, cutoff) {
        try {
            ...
            // Publish to contract
            const tx = await contract.publishSnapshot(snapshot.merkleRoot, snapshot.ipfsHash, snapshot.timestamp);
