...
class MerklePublisher {
  ...
  logNetworkInfo() {
    console.log(`\u{1F310} MerklePublisher configured: ${getNetworkDisplayName()}`);
    console.log(`\u{1F4E1} RPC URL: ${getAbstractRpcUrl().replace(/\/[^\/]+$/, "/***")}`);
    console.log(`\u{1F4CB} Registry Contract: ${this.registryAddress}`);
    if (isTestnet()) {
      console.log(`\u{1F9EA} TESTNET MODE: Safe for testing snapshot publishing`);
    } else if (isMainnet()) {
      console.log(`\u{1F680} MAINNET MODE: Publishing real snapshots to production`);
    }
  }
  /**
   * Generate merkle tree from user balances or transaction log
   */
  async generateMerkleTree(cutoff) {
    console.log(`\u{1F4CA} Generating merkle tree for ${this.network}...`);
    const useTransactionLog = process.env.MERKLE_FROM_TXLOG === "true";
    if (useTransactionLog && cutoff) {
      console.log(`\u{1F504} Using transaction log aggregation up to ${cutoff.toISOString()}`);
      return this.buildMerkleFromTransactionLog(cutoff);
    } else {
      console.log(`\u{1F4CA} Using UserBalance table (legacy mode)`);
   ...
   */
  async buildMerkleFromTransactionLog(cutoff) {
    ...
    `;
    const balanceDeltas = await prisma.$queryRawUnsafe(aggregateQuery, cutoff);
    console.log(`\u{1F4C8} Found ${balanceDeltas.length} balance delta aggregations`);
    const userBalanceMap = /* @__PURE__ */ new Map();
    const txLogBalanceMap = /* @__PURE__ */ new Map();
    ...
    const userTotalMap = /* @__PURE__ */ new Map();
    for (const delta of balanceDeltas) {
   ...
   */
  async buildMerkleFromUserBalance() {
    const userBalances = await prisma.userBalance.findMany({
      where: {
        amount: { gt: 0 }
      },
   ...
   */
  buildMerkleTreeFromBalanceMap(balanceMap, source) {
    const leaves = [];
    let totalBalance = 0n;
    for (const [address, amount] of balanceMap) {
      ...
      leaves.push({
        ...
        proof: []
        // Will be filled when tree is built
      });
      totalBalance += amount;
    }
    leaves.sort((a, b) => a.address.localeCompare(b.address));
    const merkleRoot = this.buildMerkleRoot(leaves.map(
      (l) => solidityPackedKeccak256(["address", "uint256"], [l.address, l.amount])
    ));
    console.log(`\u2705 Generated merkle tree from ${source}: ${leaves.length} users, total: ${totalBalance.toString()}`);
    console.log(`\u{1F333} Merkle root: ${merkleRoot}`);
    return {
      merkleRoot,
      leaves,
   ...
   */
  async publishToIPFS(snapshot) {
    const placeholder = `Qm${Math.random().toString(36).substring(2).padEnd(44, "0")}`;
    console.log(`\u{1F4E4} IPFS placeholder: ${placeholder}`);
    console.log(`\u{1F4C4} Snapshot data: ${JSON.stringify(snapshot, null, 2)}`);
    return placeholder;
  }
  /**
   * Publish snapshot to registry contract
   */
  async publishSnapshot(privateKey, cutoff) {
    try {
      console.log(`
\u{1F680} Starting snapshot publishing on ${this.network}...`);
      const snapshotCutoff = cutoff || /* @__PURE__ */ new Date();
      console.log(`\u{1F4C5} Snapshot cutoff: ${snapshotCutoff.toISOString()}`);
      const treeData = await this.generateMerkleTree(snapshotCutoff);
      const snapshot = {
        merkleRoot: treeData.merkleRoot,
        ipfsHash: "",
        // Will be set after IPFS upload
        timestamp: Math.floor(snapshotCutoff.getTime() / 1e3),
      ...
      console.log(`\u{1F4DD} Publishing snapshot to registry contract...`);
   ...
   */
  async storeSnapshot(snapshot, txHash, gasUsed) {
    await prisma.$transaction(async (tx) => {
      await tx.merkleSnapshot.create({
        data: {
          merkleRoot: snapshot.merkleRoot,
          ipfsHash: snapshot.ipfsHash,
          timestamp: new Date(snapshot.timestamp * 1e3),
    ...
    console.log(`\u{1F4BE} Snapshot metadata stored and transaction logged`);
  }
  /**
   * Verify snapshot exists on chain
   */
  async verifySnapshot(merkleRoot) {
    try {
      ...
      console.log(`\u{1F50D} Snapshot verification for ${merkleRoot.slice(0, 10)}...: ${isValid ? "\u2705 VALID" : "\u274C INVALID"}`);
      return isValid;
    } catch (error) {
...
const merklePublisher = new MerklePublisher();
export {
  merklePublisher
};
//# sourceMappingURL=merkle_publisher.js.map
