...
export class PIPChipsLMSR {
    lmsr;
    constructor(liquidityParameter = 1000, outcomes = ['YES', 'NO']) {
    ...
    async calculateBetCost(currentShares, outcome, pipchipsAmount) {
        const financialLog = logFinancialOperation('pipchips_bet_calculation', 'system', pipchipsAmount.toString());
        try {
            financialLog.start();
    ...
    async placeBet(userId, marketId, outcome, pipchipsAmount) {
        const financialLog = logFinancialOperation('pipchips_place_bet', userId, pipchipsAmount.toString());
        try {
            financialLog.start();
            const result = await prisma.$transaction(async (tx) => {
                ...
                const newBalance = await pipchipsService.debitPIPChips(userId, costCalc.actualCost, 'PREDICTION_BET', marketId, `Bet ${costCalc.actualCost} PIPChips on ${outcome}`, {
                    marketId,
                    outcome,
    ...
    async resolveMarket(marketId, winningOutcome, adminUserId) {
        const financialLog = logFinancialOperation('pipchips_market_resolution', adminUserId, marketId);
        try {
            financialLog.start();
            const result = await prisma.$transaction(async (tx) => {
                ...
                for (const bet of winningBets) {
                    ...
                    await pipchipsService.creditPIPChips(bet.userId, payout, 'PREDICTION_WIN', marketId, `Won ${payout} PIPChips from market resolution`, {
                        marketId,
     ...
     */
    static createInitialMarket(outcomes, liquidityParameter = 1000) {
        const shares = {};
        const prices = {};
