...
class CSRFProtectionService {
    ...
    generateToken(sessionId, userId) {
        ...
        const bindingInfo = [];
        if (sessionId)
            bindingInfo.push(`session:${sessionId.slice(0, 8)}...`);
        if (userId)
            bindingInfo.push(`user:${userId}`);
    ...
    verifyToken(token, secret, sessionId, userId) {
        ...
        try {
            ...
            if (isValid) {
                const bindingInfo = [];
                if (stored.sessionId)
                    bindingInfo.push(`session:${stored.sessionId.slice(0, 8)}...`);
                if (stored.userId)
                    bindingInfo.push(`user:${stored.userId}`);
 ...
 */
export function generateCSRFToken(sessionId, userId) {
 ...
 */
export function cleanupCSRFService() {
    csrfService.destroy();
}
