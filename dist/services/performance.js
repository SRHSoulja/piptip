class PerformanceMonitor {
  ...
  getSlowOperations(thresholdMs = 100) {
    return this.metrics.filter((m) => m.duration > thresholdMs).sort((a, b) => b.duration - a.duration).slice(0, 10);
  }
  // Clear metrics
  clear() {
    this.metrics = [];
...
function endTimer(operation, metadata) {
  return monitor.endTimer(operation, metadata);
}
function getMetricsSummary(operation) {
...
async function withTiming(operation, fn, metadata) {
  startTimer(operation);
  try {
...
function startPerformanceReporting(intervalMinutes = 5) {
  return setInterval(() => {
...
var performance_default = monitor;
export {
