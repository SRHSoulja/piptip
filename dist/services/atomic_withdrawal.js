...
async function executeAtomicWithdrawal(params, token, treasuryContract, signer) {
    ...
    const result = await prisma.$transaction(
      async (tx) => {
        ...
        if (currentBalanceAtomic < amountAtomic) {
          throw new Error(
            `Insufficient balance: has ${formatAmount(currentBalanceAtomic, token)}, requested ${formatAmount(amountAtomic, token)}`
          );
        }
        let blockchainTx;
        try {
      ...
      },
      {
        ...
        // Highest isolation level for financial operations
        timeout: 3e4,
        // 30 second timeout
        maxWait: 15e3
...
async function validateWithdrawalPreconditions(userId, tokenId, amountHuman, token, appConfig) {
  try {
...
async function performComprehensiveWithdrawalCheck(discordUserId, tokenId, amountHuman) {
  try {
...
export {
  ...
  performComprehensiveWithdrawalCheck,
  validateWithdrawalPreconditions
};
//# sourceMappingURL=atomic_withdrawal.js.map
