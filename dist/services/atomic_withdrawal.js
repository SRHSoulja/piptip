 ...
 */
export async function executeAtomicWithdrawal(params, token, treasuryContract, signer) {
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (currentBalanceAtomic < amountAtomic) {
                throw new Error(`Insufficient balance: has ${formatAmount(currentBalanceAtomic, token)}, ` +
 ...
 */
export async function validateWithdrawalPreconditions(userId, tokenId, amountHuman, token, appConfig) {
    try {
        // Check basic limits
 ...
 */
export async function performComprehensiveWithdrawalCheck(discordUserId, tokenId, amountHuman) {
    try {
        ...
        const preconditionCheck = await validateWithdrawalPreconditions(user.id, tokenId, amountHuman, token, appConfig);
