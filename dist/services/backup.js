...
export class BackupService {
    ...
    async createPrismaBackup() {
        ...
        try {
            ...
            const [users, tokens, userBalances, tips, groupTips, groupTipClaims, matches, transactions, tierMemberships, tiers, tierPrices, approvedServers, ads, appConfig] = await Promise.all([
    ...
    async getBackupStatus() {
        try {
            ...
            const backupFiles = files
                ...
                .sort()
                .reverse();
            const recentBackups = await Promise.all(backupFiles.slice(0, 10).map(async (file) => {
