...
class EmergencyControlsService {
  ...
  async emergencyShutdown(params) {
    try {
      ...
      console.error(`\u{1F6A8} EMERGENCY SHUTDOWN ACTIVATED by ${params.triggeredBy}: ${params.reason}`);
      return {
        success: true,
   ...
   */
  async cancelAllActiveMarkets(triggeredBy, reason) {
    try {
      const activeMarkets = await prisma.predictionMarket.findMany({
        where: { status: "ACTIVE" },
        select: { id: true, title: true }
      });
      console.log(`\u{1F6AB} Cancelling ${activeMarkets.length} active prediction markets due to emergency shutdown`);
  ...
  async performSystemHealthCheck() {
    ...
    try {
      try {
        ...
        checks.push({ component: "Database", status: "fail", message: "Connection failed" });
      }
      try {
        ...
        if (negativeBalances > 0) {
          ...
          checks.push({ component: "Balance Integrity", status: "pass" });
        }
      } catch (error) {
        checks.push({ component: "Balance Integrity", status: "fail", message: "Check failed" });
      }
      try {
        const activeMarkets = await prisma.predictionMarket.count({
          where: { status: "ACTIVE" }
        });
        if (activeMarkets > 1e3) {
          checks.push({
          ...
          checks.push({ component: "Market Load", status: "pass" });
        }
      } catch (error) {
        checks.push({ component: "Market Load", status: "fail", message: "Check failed" });
      }
      try {
        ...
        if (recentErrors > 50) {
          ...
          checks.push({ component: "Error Rate", status: "pass" });
        }
      } catch (error) {
        checks.push({ component: "Error Rate", status: "fail", message: "Check failed" });
      }
    } catch (error) {
      console.error("Health check failed:", error);
      checks.push({ component: "System", status: "fail", message: "Health check system failure" });
    }
   ...
   */
  async logEmergencyAction(type, data, triggeredBy) {
    try {
   ...
   */
  async forceKillSwitch(triggeredBy, reason) {
    try {
      ...
      console.error(`\u{1F6A8}\u{1F6A8}\u{1F6A8} FORCE KILL SWITCH ACTIVATED by ${triggeredBy}: ${reason}`);
      return {
        success: true,
...
async function loadAchievementsFlag() {
  try {
...
async function loadStreakProtectionFlag() {
  try {
    const config = await prisma.appConfig.findFirst({
...
async function areAchievementsEnabled() {
  const now = Date.now();
...
async function isStreakProtectionEnabled() {
  const now = Date.now();
...
const emergencyControls = new EmergencyControlsService();
export {
...
//# sourceMappingURL=emergency_controls.js.map
