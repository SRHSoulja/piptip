        ...
        try {
            ...
            for (const user of users) {
                ...
                try {
                    ...
                    const activeMemberships = user.tierMemberships.filter(m => m.status === 'ACTIVE' && m.expiresAt > new Date());
                    ...
                    const highestTier = activeMemberships.reduce((highest, current) => current.tierId > highest.tierId ? current : highest);
...
export class TierRoleSyncService {
    ...
    static SYNC_INTERVAL_HOURS = 6; // Sync every 6 hours
