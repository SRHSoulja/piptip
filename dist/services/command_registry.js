...
async function getApprovedGuildIds() {
    const rows = await prisma.approvedServer.findMany({
        where: { enabled: true },
        select: { guildId: true },
    });
    const ids = rows.map(r => r.guildId);
    if (ids.length === 0 && FALLBACK_GUILD_ID)
        return [FALLBACK_GUILD_ID];
    return ids;
}
export async function registerCommandsForApprovedGuilds(commandsJson) {
    const guildIds = await getApprovedGuildIds();
    const rest = new REST({ version: "10" }).setToken(TOKEN);
    for (const gid of guildIds) {
        await rest.put(Routes.applicationGuildCommands(CLIENT_ID, gid), { body: commandsJson });
