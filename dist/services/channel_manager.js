...
export async function checkChannelPermissions(interaction, commandCategory = 'general') {
...
export async function logChannelActivity(guildId, channelId, commandType, commandName, userId, success = true) {
    try {
...
export async function getChannelActivity(guildId, hours = 24) {
    ...
    try {
        ...
        for (const activity of activities) {
            ...
            if (activity.success)
                channelStats[activity.channelId].success++;
...
export async function suggestChannelsForGuild(guild) {
    const suggested = [];
    const autoCreate = [];
