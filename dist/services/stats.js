...
class TimeStatsCache {
  ...
  invalidate(key) {
    ...
    console.log(`\u{1F5D1}\uFE0F Invalidated time stats cache${key ? ` for ${key}` : " (all keys)"}`);
  }
...
setInterval(() => {
  timeStatsCache.cleanup();
}, 15 * 60 * 1e3);
class StatsService {
  async getBotStats() {
    console.log("\u{1F504} Generating bot statistics...");
    const [
      kpis,
      serverBreakdown,
      tokenBreakdown,
  ...
  async getServerBreakdown() {
    ...
    let serverStats = guildIds.map((guildId) => {
      ...
      return {
        guildId,
        serverName: `Server ${guildId}`,
        // Will be updated with real names
  ...
  async getTokenBreakdown() {
    ...
    const tokenStatsPromises = tokens.map(async (token) => {
      ...
      const avgTipSize = tipCount > 0 && totalTippedDecimal ? totalTippedDecimal.dividedBy(tipCount).toString() : "0";
      return {
        tokenId: token.id,
        symbol: token.symbol,
  ...
  async getGlobalStats() {
    ...
    const avgTipSize = totalTipCount > 0 && totalTipAmountDecimal ? totalTipAmountDecimal.dividedBy(totalTipCount).toString() : "0";
    return {
      totalTipAmount,
      totalTipCount,
  ...
  async getHighlights() {
    ...
    return {
      biggestTip: biggestTip ? {
        amount: biggestTip.amountAtomic.toString(),
        // This is already correct, single tip amount
  ...
  async getTimeBreakdown() {
    ...
    if (cached) {
      console.log("\u{1F4CA} Using cached time breakdown data");
      return cached;
    }
    console.log("\u{1F504} Generating time breakdown with grouped queries...");
    const now = /* @__PURE__ */ new Date();
    const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1e3);
    const fourWeeksAgo = new Date(now.getTime() - 4 * 7 * 24 * 60 * 60 * 1e3);
    ...
    const dailyTipMap = /* @__PURE__ */ new Map();
    ...
    const weekly = Array.from({ length: 4 }, (_, i) => {
      const weekStart = new Date(now.getTime() - (i + 1) * 7 * 24 * 60 * 60 * 1e3);
