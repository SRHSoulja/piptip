...
class ApiKeyManagementService {
    ...
    async validateApiKey(key, req) {
        ...
        if (apiKey.allowedEndpoints && apiKey.allowedEndpoints.length > 0) {
            const isEndpointAllowed = apiKey.allowedEndpoints.some(endpoint => req.path.startsWith(endpoint) || req.path.match(new RegExp(endpoint)));
    ...
    apiKeyMiddleware(requiredPermissions = []) {
        return async (req, res, next) => {
            ...
            if (requiredPermissions.length > 0) {
                const hasPermission = requiredPermissions.every(perm => validation.apiKey.permissions.includes(perm) ||
...
export const apiKeyMiddleware = (permissions = []) => apiKeyManager.apiKeyMiddleware(permissions);
