...
export class PIPChipsService {
    ...
    async processTransaction(transaction) {
        const financialLog = logFinancialOperation(`pipchips_${transaction.type.toLowerCase()}`, transaction.userId, transaction.amount.toString());
        try {
            financialLog.start();
    ...
    async initializeNewUser(discordId) {
        ...
        await this.creditPIPChips(discordId, BigInt(startingBalance), 'STARTING_BONUS', undefined, 'Welcome bonus for new user');
        logger.info({
            userId: discordId,
    ...
    async getSystemStats() {
        const [totalUsers, totalCirculation, totalEarned, totalSpent, totalBought, activeUsers24h, dailyClaims24h] = await Promise.all([
            prisma.user.count(),
