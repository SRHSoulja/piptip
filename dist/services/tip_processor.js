...
export async function processTip(data, client) {
    try {
        ...
        const bestTaxBenefit = await RoleTaxBenefitService.getBestTaxBenefit(fromUser.id, data.guildId || '', data.userId);
        ...
        if (data.tipType === "direct") {
            ...
            if (bestTaxBenefit && bestTaxBenefit.exemptionRate > 0 && taxSavedAtomic > 0n) {
                ...
                const benefitMessage = await formatBenefitNotification(bestTaxBenefit, 'tax', formatAmount(taxSavedAtomic, token), token.symbol, data.guildId);
            ...
            try {
                ...
                if (bestTaxBenefit && bestTaxBenefit.exemptionRate > 0) {
                    ...
                    if (savedAmount > 0n) {
                        const benefitEmoji = bestTaxBenefit.source.startsWith('tier:') ? '⭐' : '🎭';
                        const benefitMessage = await formatBenefitNotification(bestTaxBenefit, 'tax', formatAmount(savedAmount, token), token.symbol, data.guildId);
                        detailsMessage += `\n${benefitEmoji} ${benefitMessage}`;
                    }
                }
                return {
