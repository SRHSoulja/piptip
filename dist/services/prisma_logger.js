...
function extractOperationName(query) {
    ...
    if (sql.startsWith('select')) {
        if (sql.includes('from "tip"'))
            return 'TIP_READ';
        if (sql.includes('from "grouptip"'))
            return 'GROUP_TIP_READ';
        if (sql.includes('from "grouptipclaim"'))
            return 'CLAIM_READ';
        if (sql.includes('from "userbalance"'))
            return 'BALANCE_READ';
        if (sql.includes('from "user"'))
            return 'USER_READ';
        if (sql.includes('from "token"'))
            return 'TOKEN_READ';
    ...
    if (sql.startsWith('insert')) {
        if (sql.includes('into "tip"'))
            return 'TIP_CREATE';
        if (sql.includes('into "grouptip"'))
            return 'GROUP_TIP_CREATE';
        if (sql.includes('into "grouptipclaim"'))
            return 'CLAIM_CREATE';
        if (sql.includes('into "userbalance"'))
            return 'BALANCE_CREATE';
        if (sql.includes('into "user"'))
            return 'USER_CREATE';
        if (sql.includes('into "transaction"'))
    ...
    if (sql.startsWith('update')) {
        if (sql.includes('"tip"'))
            return 'TIP_UPDATE';
        if (sql.includes('"grouptip"'))
            return 'GROUP_TIP_UPDATE';
        if (sql.includes('"grouptipclaim"'))
            return 'CLAIM_UPDATE';
        if (sql.includes('"userbalance"'))
    ...
    if (sql.startsWith('delete')) {
        if (sql.includes('from "tip"'))
            return 'TIP_DELETE';
        if (sql.includes('from "grouptip"'))
            return 'GROUP_TIP_DELETE';
        if (sql.includes('from "grouptipclaim"'))
            return 'CLAIM_DELETE';
        if (sql.includes('from "userbalance"'))
            return 'BALANCE_DELETE';
        return 'DELETE_QUERY';
    }
...
export function taggedPrismaOperation(operation, operationName) {
...
export async function healthCheckQuery(timeoutMs = 200) {
    try {
