  ...
  async fetchUpcomingGames(league) {
    try {
      ...
      const upcomingGames = data.events.filter((event) => {
        ...
        const gameDate = /* @__PURE__ */ new Date(`${event.dateEvent} ${event.strTime}`);
  ...
  async resolveSportsMarket(market) {
    ...
    switch (marketType) {
      case "SPORTS_WINNER":
        ...
        const isHomeTeam = result.homeTeam.toLowerCase().includes(betTeam.toLowerCase()) || betTeam.toLowerCase().includes(result.homeTeam.toLowerCase());
      ...
      case "SPORTS_SPREAD":
        ...
        const isSpreadHome = result.homeTeam.toLowerCase().includes(spreadTeam.toLowerCase()) || spreadTeam.toLowerCase().includes(result.homeTeam.toLowerCase());
        let actualSpread;
        if (isSpreadHome) {
...
const sportsResolver = new SportsResolverService();
export {
  SportsResolverService,
  sportsResolver
};
//# sourceMappingURL=sports_resolver.js.map
