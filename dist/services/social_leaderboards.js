...
export async function calculateSocialScore(discordId) {
    try {
...
export async function getSocialLeaderboard(limit = 10) {
    try {
...
export async function getGenerosityLeaderboard(limit = 10) {
    try {
        const users = await prisma.user.findMany({
            include: {
                tipsSent: {
...
export function createLeaderboardEmbed(type, data, userRank) {
    const configs = {
        social: {
...
export async function getUserRank(discordId, type) {
    try {
        let leaderboard = [];
        switch (type) {
            case 'social':
