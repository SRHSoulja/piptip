...
class CDNManager {
    ...
    `;
        ...
        // Replace CSS links
        optimizedContent = optimizedContent.replace(/<link[^>]+href="([^"]+\.css)"[^>]*>/g, (match, cssPath) => {
        ...
        optimizedContent = optimizedContent.replace(/<script[^>]+src="([^"]+\.js)"[^>]*>/g, (match, jsPath) => {
            const cdnUrl = this.getAssetUrl(jsPath);
            return match.replace(jsPath, cdnUrl);
        });
        // Replace image sources with responsive images
        optimizedContent = optimizedContent.replace(/<img[^>]+src="([^"]+)"[^>]*>/g, (match, imgPath) => {
        ...
        optimizedContent = optimizedContent.replace('</head>', `${resourceHints}\n</head>`);
...
export const getAssetUrl = (path) => cdnManager.getAssetUrl(path);
export const getImageUrl = (path, options) => cdnManager.getImageUrl(path, options);
export const getCacheHeaders = (path, contentType) => cdnManager.getCacheHeaders(path, contentType);
export const optimizeHTML = (content) => cdnManager.generateOptimizedHTML(content);
