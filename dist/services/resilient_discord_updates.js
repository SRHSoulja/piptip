...
class ResilientDiscordUpdateService {
    ...
    IMMEDIATE_RETRY_WINDOW_MS = 30000; // Try immediate retries for 30 seconds
    constructor() {
    ...
    async executeGroupTipUpdate(update) {
        ...
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Discord update timeout")), 15000));
        try {
 ...
 */
export async function updateGroupTipMessageResilient(client, tipId, maxAttempts = 8) {
    console.log(`🚀 Queuing resilient Discord update for tip ${tipId}`);
    return await resilientDiscordUpdates.queueUpdate("group_tip_finalize", tipId, maxAttempts, true // Use exponential backoff
    );
}
/**
 * Initialize the resilient Discord update service
 */
export async function initializeResilientDiscordUpdates(client) {
    await resilientDiscordUpdates.initialize(client);
}
/**
 * Shutdown the resilient Discord update service
 */
export async function shutdownResilientDiscordUpdates() {
