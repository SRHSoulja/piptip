    ...
    async fetchDexScreenerTopTokens(chain) {
        try {
            ...
            for (const token of boostedTokens) {
                ...
                if (!validChains.includes(token.chainId?.toLowerCase()))
                    continue;
                        ...
                        if (detailResponse.ok) {
                            ...
                            if (detailData.pairs && Array.isArray(detailData.pairs)) {
                                ...
                                const bestPair = detailData.pairs
                                    .filter((p) => validChains.includes(p.chainId?.toLowerCase()))
                    ...
                    try {
                        ...
                        if (searchResponse.ok) {
                            ...
                            if (searchData.pairs && Array.isArray(searchData.pairs)) {
                                const chainPairs = searchData.pairs.filter((p) => chain.toLowerCase() === 'all' || p.chainId?.toLowerCase() === chain.toLowerCase());
                                if (chainPairs.length > 0) {
                                    const bestPair = chainPairs.sort((a, b) => (b.volume?.h24 || 0) - (a.volume?.h24 || 0))[0];
