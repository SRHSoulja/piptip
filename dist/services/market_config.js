...
class MarketConfigService {
  ...
  getDefaultConfig() {
    return {
      settings: {
        defaultRakePercentage: 3,
        minBet: 1,
        maxBet: 1e4,
        minMarketDuration: 3600,
        // 1 hour
        maxMarketDuration: 604800,
        // 1 week
        autoResolveEnabled: true,
        autoResolveInterval: 3e5,
        // 5 minutes
  ...
  getMaxMarketSize(token) {
    const config = this.getConfig();
    return config.riskLimits.maxTotalMarketSize[token.toUpperCase()] || 1e5;
  }
  /**
  ...
  getSportsMarketTypes() {
    const config = this.getConfig();
    return Object.entries(config.templates).filter(([key]) => key.startsWith("SPORTS_")).map(([key, template]) => ({
      value: key,
      name: template.name,
      description: template.description
    }));
...
const marketConfig = new MarketConfigService();
export {
  MarketConfigService,
  marketConfig
};
//# sourceMappingURL=market_config.js.map
